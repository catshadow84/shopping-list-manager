...
class ShoppingListApp:
    ...
    def view_list(self):
        ...
        for item_dict in self.shopping_list:
            if item_dict['category'] != current_category:
                if current_category is not None:
                    list_str += "\n"
                list_str += f"{item_dict['category']}:\n"
                current_category = item_dict['category']
            list_str += f"  {item_dict['item']}\n"
        
        messagebox.showinfo("Shopping List", list_str.strip())
    ...
    def add_item(self):
        ...
        category = simpledialog.askstring("Add Item", "Enter the category for the item: ")
        if not category:
            return
        
        self.shopping_list.append({'item': item, 'category': category})
        self.shopping_list.sort(key=lambda x: (x['category'], x['item']))
        messagebox.showinfo("Shopping List", f"{item} has been added to your shopping list under {category}.")
    ...
    def remove_item(self):
        ...
        items_display = "\n".join(
            f"{index+1}. {item['item']} ({item['category']})"
            for index, item in enumerate(self.shopping_list)
        )
        
        item_index = simpledialog.askinteger(
            "Remove Item", 
            f"Select the number of the item to remove:\n\n{items_display}"
        )
        
        if (item_index is not None and 
            1 <= item_index <= len(self.shopping_list)):
            removed_item = self.shopping_list.pop(item_index - 1)
            messagebox.showinfo("Shopping List", 
                               f"{removed_item['item']} has been removed from your shopping list.")
        else:
